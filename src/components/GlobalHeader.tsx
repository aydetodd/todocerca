    ...
    <header className="bg-card shadow-sm border-b border-border sticky top-0 z-50">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          {isProvider && (
            <div className="flex items-center gap-1 bg-muted/50 rounded-full px-2 py-1 border border-border">
              <button
                onClick={() => updateStatus('available')}
                disabled={loading}
                className={`
                  w-6 h-6 rounded-full transition-all duration-200 border
                  ${status === 'available'
                    ? 'bg-green-500 border-green-300 shadow-[0_0_10px_rgba(34,197,94,0.8)] scale-110'
              ...
              <button
                ...
                className={`
                  w-6 h-6 rounded-full transition-all duration-200 border
                  ${status === 'busy'
                    ? 'bg-yellow-400 border-yellow-200 shadow-[0_0_10px_rgba(234,179,8,0.8)] scale-110'
              ...
              <button
                ...
                className={`
                  w-6 h-6 rounded-full transition-all duration-200 border
                  ${status === 'offline'
                    ? 'bg-red-500 border-red-300 shadow-[0_0_10px_rgba(239,68,68,0.8)] scale-110'
