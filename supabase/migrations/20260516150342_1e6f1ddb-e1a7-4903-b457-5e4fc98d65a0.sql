...
BEGIN
  ...
  route_users AS (
    SELECT *
    FROM latest_assignment_per_driver lad
    WHERE lad.producto_id = _producto_id
  )
  SELECT
    ru.user_id,
...
GRANT EXECUTE ON FUNCTION public.get_route_live_units(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_route_live_units(uuid) TO anon;