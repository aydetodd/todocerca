...
BEGIN
  ...
  IF public.cuenta_esta_bloqueada(v_uid) THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Tu cuenta está bloqueada por seguridad. Desbloquéala desde tu teléfono.');
  END IF;
  ...
  IF public.cuenta_esta_bloqueada(v_to_titular) THEN
    RETURN jsonb_build_object('ok', false, 'error', 'La cuenta destino está bloqueada por seguridad.');
  END IF;
