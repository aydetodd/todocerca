...
CREATE FUNCTION public.find_user_by_phone(phone_param text)
RETURNS TABLE(user_id uuid, apodo text, nombre text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT p.user_id, p.apodo, p.nombre
  FROM public.profiles p
  WHERE public.normalize_phone(phone_param) <> ''
    AND (public.normalize_phone(p.telefono) = public.normalize_phone(phone_param)
      OR public.normalize_phone(p.phone) = public.normalize_phone(phone_param))
  LIMIT 1;
$$;
GRANT EXECUTE ON FUNCTION public.find_user_by_phone(text) TO authenticated;

CREATE FUNCTION public.add_contact_by_phone(phone_param text)
