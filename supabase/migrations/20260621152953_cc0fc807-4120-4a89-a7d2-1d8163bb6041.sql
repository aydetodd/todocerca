CREATE OR REPLACE FUNCTION public.get_public_routes_with_live_units(_estado text DEFAULT NULL::text, _ciudad text DEFAULT NULL::text, _route_type text DEFAULT 'urbana'::text)
RETURNS TABLE(id uuid, nombre text, unit_count integer)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  WITH active_assignment_per_driver AS (
    SELECT DISTINCT ON (pf.user_id)
      pf.user_id,
      pf.active_route_producto_id AS producto_id,
      pf.active_unidad_id AS unidad_id
    FROM public.profiles pf
    JOIN public.choferes_empresa ce ON ce.id = pf.active_chofer_id AND ce.user_id = pf.user_id
    JOIN public.unidades_empresa ue ON ue.id = pf.active_unidad_id
    JOIN public.productos pp ON pp.id = pf.active_route_producto_id
    WHERE pf.estado IN ('available', 'busy')
      AND pf.active_route_producto_id IS NOT NULL
    ...
    ORDER BY pf.user_id, pf.updated_at DESC
  ),
  fallback_latest_assignment_per_driver AS (
    ...
    WHERE ce.is_active = true
      ...
      AND COALESCE(pp.route_type, 'urbana') = COALESCE(NULLIF(_route_type, ''), 'urbana')
      AND COALESCE(pp.is_private, false) = false
      AND pf.route_name IS NOT NULL
      AND pf.active_route_producto_id IS NULL
  ...
  assignment_per_driver AS (
    SELECT * FROM active_assignment_per_driver
    UNION ALL
    SELECT * FROM fallback_latest_assignment_per_driver
  ),
  live_products AS (
    SELECT
      p.id,
      ...
      apd.user_id
    FROM assignment_per_driver apd
    JOIN public.productos p ON p.id = apd.producto_id
    JOIN public.proveedor_locations pl ON pl.user_id = apd.user_id
    JOIN public.profiles pf ON pf.user_id = apd.user_id
...
AS $function$
  ...
  active_assignment_per_driver AS (
    SELECT DISTINCT ON (pf.user_id)
      pf.user_id,
      pf.active_route_producto_id AS producto_id,
      pf.active_unidad_id AS unidad_id,
    ...
    WHERE pf.estado IN ('available', 'busy')
      ...
      AND COALESCE(ue.transport_type, COALESCE(ce.transport_type, 'publico')) <> 'taxi'
      AND COALESCE(pp.is_private, false) = false
      AND COALESCE(pp.route_type, 'urbana') IN ('urbana', 'foranea')
    ORDER BY pf.user_id, pf.updated_at DESC
  ),
  fallback_latest_assignment_per_driver AS (
    SELECT DISTINCT ON (ce.user_id)
  ...
  route_users AS (
    SELECT aa.user_id, aa.producto_id, aa.unidad_id, aa.driver_name
    FROM active_assignment_per_driver aa
    JOIN matching_products mp ON mp.id = aa.producto_id
    UNION ALL
    SELECT lta.user_id, lta.producto_id, lta.unidad_id, lta.driver_name
    FROM fallback_latest_assignment_per_driver lta
