...
BEGIN
  ...
  WHERE p.id = _producto_id
    AND (p.route_type IN ('privada','foranea') OR p.is_private IS TRUE)
    AND EXISTS (
      SELECT 1
...
BEGIN
  ...
  IF _updated_id IS NULL THEN
    RAISE EXCEPTION 'No se quitó: esta ruta no pertenece a tu concesionario o no es privada/foránea.';
  END IF;
